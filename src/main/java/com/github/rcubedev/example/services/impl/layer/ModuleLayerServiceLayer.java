...
import com.github.rcubedev.example.services.api.exception.ServiceSignatureException;
...
import com.github.rcubedev.example.services.impl.EagerServiceImpl;
import com.github.rcubedev.example.services.impl.ProviderServiceImpl;
...
/**
 * {@link ServiceLayer} backed by a {@link ModuleLayer}.
 * <p>
 * Uses {@link ServiceLoader#load(ModuleLayer, Class)}.
 */
public final class ModuleLayerServiceLayer implements ServiceLayer {

    private final String name;
    private final ModuleLayer moduleLayer;
    private final int priority;

    public ModuleLayerServiceLayer(@NotNull String name, @NotNull ModuleLayer moduleLayer, int priority) {
        this.name = name;
...
//                .map(ProviderServiceImpl::new);
        ...
        Stream<Service<S>> stream = ServiceLoader.load(this.moduleLayer, contract).stream()
                .map(ProviderServiceImpl::new);
...
//        Stream<ServiceLoader.Provider<S>> stream = ServiceLoader.load(contract).stream();
        ...
        List<ServiceLoader.Provider<S>> providers = ServiceLoader.load(this.moduleLayer, contract)
                .stream()
                .toList();
    ...
    public String toString() {
        return "ModuleLayerServiceLayer[name=" + this.name + ", priority=" + this.priority + "]";
    }
}