...
public final class RecursionGuard {
    // todo(jdk25): scoped values. not sure how #bypass will work tho
    ...
    public void run(@NotNull Runnable action) throws EventStackOverflowException {
        int[] d = depth.get();
        int current = d[0];
        ...
        try {
            ...
            d[0] = current;
            //if ((d[0] = previousValue) == 0) depth.remove(); // don't remove: lightweight int[]
        }
    }
